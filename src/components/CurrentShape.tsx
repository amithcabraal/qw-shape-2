...
export function CurrentShape({ shape, isDraggable = true }: CurrentShapeProps) {
  ...
  const [{ isDragging }, drag] = useDrag(() => ({
    ...
    item: (monitor) => {
      ...
      // Add extra vertical offset for touch devices
      const isTouchDevice = 'ontouchstart' in window;
      const touchOffset = isTouchDevice ? 3 : 0; // Offset by 3 cells up for touch devices

      return {
        shape,
        offset: { 
          x: offsetX, 
          y: offsetY - firstFilledY - touchOffset 
        }
      };
    },
    collect: (monitor) => ({
